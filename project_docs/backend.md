...
- 外部响应不得未经验证直接改变已接受的 `BriefItem`、`DraftDecision` 或 `DraftExecution`；内容 LLM 必须返回 `title` 和零至两个摘要展示目标，Python 生成完整 `EvidenceBinding.claim`。`BriefItem.brief_mode=title_only` 要求空 brief，`expanded` 要求一至两句非空 brief；摘要引用只在原始标题范围内时逐句删除。质量 LLM 不可用或响应无效时，确定性事实规则通过的条目必须使用 `rules_only` 自动入选；跨语言自动降级还必须有逐字实体锚点，且不能包含规则无法核验的用途等翻译语义。英文来源的确定性标题回退只允许已登记机构/模型、`@handle`、数字/版本等受控细节，不得保留任意普通英文名词或描述性形容词。`source_fallback_used` 独立记录，不能覆盖原始技术失败原因。不得请求人工复核或 LLM 修正事实。
- 候选进入聚类和内容 LLM 前，必须根据规范来源证据统一分类并冻结 `content_type`。分类器明确拒绝或证据无效时终止；普通来源预检失败仅降低候选顺序。`ai_update` 可接受同一 quote 支持的非数字能力演示或工作流进展，但不能被 LLM 升级为正式发布；分类审计只写私有 `docs/debug/<date>-briefing.json`，不得进入 `latest.json`、页面或微信草稿。
...
- 质量审计应区分 `missing_target_binding`、`quote_not_found`、`source_url_mismatch`、`protected_token_missing`、`action_not_supported`、`semantic_review_rejected`、`quality_llm_unavailable` 和 `quality_llm_invalid_response`，不能把所有失败压缩为笼统的 `unsupported_claim`。内容生成审计还要区分 `content_llm_timeout`、`content_llm_unavailable`、`invalid_builder_response`、`builder_item_missing`、`builder_item_malformed`、`builder_item_duplicate` 与有效结构下的 `translation_failed`；中文原文或仅含已登记实体/完整源标题主体、动作和原文细节锚点的确定性 title_only 回退以独立 `source_fallback_used` 标志记录，保留触发回退的原因码。对可确定的最终拒绝，审计附加 `reject_class=source_quality|pipeline_loss`，并汇总两类计数；歧义拒绝不得猜测归类。摘要轨迹记录输入、删除句、最终 brief、模式和原因。供应商可返回空字符串 brief，或一至两项非空字符串列表并由适配层机械拼接；三项以上或其它对象结构仍记为 `builder_item_malformed`。`_strict_item` 对每个畸形分支返回稳定的结构分类，写入私有审计的 `build.malformed_detail`（如 `item_not_object`、`missing_fields:`/`unexpected_fields:`、`index_not_int`、`index_mismatch`、`event_key_mismatch`、`title_empty`、`brief_type_invalid`、`display_target_count_invalid`、`evidence_targets_invalid`、`binding_fields_invalid`、`binding_target_empty`、`binding_target_unknown`、`binding_quote_id_empty`、`title_quote_id_unresolved`、`title_binding_missing`）；该字段只携带稳定类别、绝不写入原始模型响应或来源文本，也不进入公开产物。`_strict_item` 的返回类型因此是 `(BuiltBrief | None, str)`，`BuildResult.malformed_detail` 仅在 `reason_code == "builder_item_malformed"` 时非空。
...
- 分类拒绝的顶层原因码是既有契约，不得随意增删；否则不可区分的拒绝必须在私有审计给出有界子原因。`PublishabilityResult.rejection_detail` / `ContentClassification.rejection_detail` 目前只用于 `non_news_content`，取值为 `instructional_content`（命中 `_NON_NEWS_PATTERNS`）或 `no_asserted_action`（`_claim_frame` 无法构造已断言事件框架，即标题缺少动作词）；非该原因码的拒绝与已接受结果该字段为空。采集层将其写入候选审计的 `classification_rejection_detail`。该字段只携带稳定类别，绝不写入原始响应或来源文本，且不得进入 `latest.json`、公开 HTML、微信草稿或 `DraftDecision` 载荷。
