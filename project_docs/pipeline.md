...
1.5 事实新闻与署名观点候选
   -> 从唯一事件/动态/观点生成最多 20 条简报，先保证至少 3 条事实新闻；动态和观点目标为 5，硬上限各为 8
...
2.6 决策与确定性封面输入
   -> DraftDecision 是唯一 create|block 决策：5-20 条唯一有效简报、至少 3 条事实、动态/观点分别最多 8 条、无遗留重复，且 X 规范来源最多 8 条；目标短缺不阻断
...
`DraftDecision` 是唯一生产决策，动作只能是 `create` 或 `block`。它依据 5-20 条唯一、已核验的内容，要求至少 3 条事实新闻、动态和署名观点各最多 8 条、每位观点作者最多 1 条，每个声明拥有规范来源证据，并限制最终把 X 用作规范来源的条目不超过 8 条。`DAILY_X_TARGET_ITEMS` 只控制候选尝试顺序，不是 `DraftDecision` 的来源配额；少于软目标不会 block。少于 5 条时 block；5-19 条是正常短版。
