...
| 内容 LLM 超时、不可用、429/5xx 或整包无效、条目缺失/畸形/重复 | 是 | SDK 不自动重试；429/5xx/网关不可用立即打开本期熔断，后续条目直接降级，避免逐条等待；其它可重建错误最多重建一次。X 使用携带原因和保护锚点的单条请求，其它来源保持有界批量；失败项记录精确原因，中文原文回退另记独立标志且不覆盖原因 |
