...
| 内容 LLM 超时、不可用、429/5xx 或响应无效、条目缺失/畸形/重复 | 是 | 所有初次生成和重建均为单条请求且 SDK 不自动重试；429/5xx/网关不可用立即打开本期熔断，后续条目直接降级，避免继续逐条撞击供应商；其它可重建错误最多重建一次。X 重建还须携带失败原因和保护锚点；失败项记录精确原因，中文原文回退另记独立标志且不覆盖原因 |
