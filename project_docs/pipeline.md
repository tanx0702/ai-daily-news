...
2. 生成与核验事实简报
   ...
   -> 同语言绑定失败后才回退到跨语言等价判定（`_cross_language_display_bound`），不替换原判定。跨语言判定按句子执行、不得跨句拼接，并要求：(1) claim 为中文且原文为英文；(2) claim 的拉丁锚点集合是**同一条**原文句子的子集；(3) 分词后（最长优先，不得把词切成碎片）每个中文词要么是受控动作/虚词 `CROSS_LANGUAGE_RULE_ONLY_MARKERS`，要么命中共享中英对应表 `CROSS_LANGUAGE_NOUN_EQUIVALENTS` 且其英文对应词出现在**同一条**原文句子中；(4) 未命中的中文残留一律拒绝。该表只收高频新闻名词（研究院/辩论/合作伙伴关系/代理/功能/云端/模型/安全/开放权重等），机构名、人名、模型名、数字、版本号一律不得入表，必须继续依赖拉丁锚点；中英对应表与动作词表同为共享入口，不得在 validator 或分类层各自维护漂移副本
