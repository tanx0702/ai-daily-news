...
2. 生成与核验事实简报
   ...
   -> 第二次失败若使用完整中文原文回退，独立记录 source_fallback_used，并保留触发回退的原始原因码；若第二次重建仍为 title_claim_not_source_bound，且英文源标题可机械抽取已登记主体、动作和另一原文锚点时，允许生成只含这些锚点的 title_only 回退，不翻译对象或补写摘要
