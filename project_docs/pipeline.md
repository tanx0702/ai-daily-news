...
2. 生成与核验事实简报
   ...
   -> 确定性规则核验展示目标、来源 URL、逐字证据引文、名称/数字/动作和唯一事件；候选预检与最终 Validator 共用按 `content_type` 的来源发布性分派，发布性、绑定和跨语言 rules_only 校验共享同一动作词表，不能各自维护漂移版本；英语 `available` 可译为“可用”，且仅允许“在原文产品名中/上可用”的受控中文语序，产品名必须逐字保留
