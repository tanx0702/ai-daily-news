...
通过上述过滤的 X 候选按确定性顺序分类：先判断合格的署名观点并标为 `attributed_opinion`；观点除白名单、原帖、上下文和长度要求外，还必须同时命中明确 AI 主题与实质立场，体育、政治和生活观点不得因作者在白名单中而升级。否则，只要正文包含明确的硬新闻动作（例如发布、上线、`is live`、`releasing`、收购或任命），就保持 `fact_event`；只有在没有这类动作时，才将同时包含可机械核验的模型/版本、数字结果（如百分比、排名或速度）或具体技术进展的内容标为 `ai_update`。新增动作表达仍须通过主体、事件细节、逐条 quote 和规范 URL 门禁；推广、转发、纯链接和泛评论即使含有技术词或数字，也不得升级为 `ai_update`，不满足动态条件的候选保持 `fact_event` 或在前置过滤中被跳过。
