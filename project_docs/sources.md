...
生产 collector 只会为命中该注册表的记录写入内部 `x_source_name`，规范证据也只在 `trusted_x_collector=True` 时用它恢复真实发布者名称。未注册快照记录不能用自由文本覆盖通用发布者 `X` 或升级可信身份。
...
- 候选标题由 `_snapshot_title` 生成，保留完整推文文本，**不按字符数硬截断**。早期实现截到 180 字符，长推文标题会停在断词并导致译文漏句，因此改为完整保留：完整的长标题比整齐的残片更有用，且文本长度在上游已由 summary 上限约束。
- 转换前会确定性跳过转发/`RT @...`、招聘课程报名和祝贺推广等非独立新闻内容，并记录跳过原因；这些内容不能在观点预检失败后回落成 `fact_event`。

通过上述过滤的 X、RSS、HN、专业媒体等候选都根据规范来源证据统一分类并冻结类型：正式发布、融资、收购、论文发布、法律裁决、人事变动和行业联合行动等明确事件优先归为 `fact_event`；合格自然人原帖归为 `attributed_opinion`；其余具备明确模型/项目主体及具体进展的内容归为 `ai_update`。动态可以使用百分比、排名、速度等数值证据，也可以使用受控的能力演示、实测、支持、生成、运行或工作流进展，但必须在同一条 quote 中绑定主体、行为和能力对象。推广、转发、纯链接、教程、未证实传闻和泛评论在分类阶段拒绝；其它预检失败只降低尝试顺序，不能绕过最终门禁。观点的白名单、原帖、AI 主题和实质立场要求不变，体育、政治和生活观点不得因作者在白名单中而升级；最终观点统一为保留作者归因和标题 quote 的 `title_only`，不生成跨句扩展摘要。

快照读取失败、schema 不合法、时间过期或单条记录不完整时只跳过对应 X 候选/整份 X 快照，不影响 RSS、HN、GitHub、HF 和 arXiv。有效的本机空快照是权威结果，不会被旧的远程快照覆盖；本机文件失效才允许回退远程来源。X 候选仍须经过事件聚类和规范来源证据绑定；歧义重复项隔离且不得回填。`DAILY_X_TARGET_ITEMS` 只在未达到软目标时提高 X 候选的尝试顺序，不能绕过质检或硬凑；`ai_update` 和 `attributed_opinion` 的类别目标同样只影响尝试顺序，硬上限由选择器和 `DraftDecision` 执行。认证网页自动化仅用于短期试运行，账号、Cookie、SQLite 会话和代理配置必须保存在服务器 root 私有目录，试运行结束后删除并恢复 GitHub 快照路径。
