...
```bash
node ".agents/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_1_result "confirmed: <one-line summary>"
node ".agents/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_1_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
