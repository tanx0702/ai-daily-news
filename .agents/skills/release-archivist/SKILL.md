...
- Run `node ".agents/spec-superflow/scripts/spec-superflow.mjs" audit <change-dir>` — include `decision-point-audit.md` in archive

### DP-6 (Verification Outcome)
```bash
node ".agents/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_6_result "<pass|conditional|fail>: <summary>"
node ".agents/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_6_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
```bash
node ".agents/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> test_result pass
```

### DP-7 (Archive Confirmation)
```bash
node ".agents/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_7_result "confirmed: <archive summary>"
node ".agents/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_7_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
Run `node ".agents/spec-superflow/scripts/spec-superflow.mjs" state transition <change-dir> closing`. If delta specs exist, route to `spec-merger`.
...
- **Missing files**: If audit can't generate, run `node ".agents/spec-superflow/scripts/spec-superflow.mjs" audit` manually
