                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "content_type 为 fact_event 和 ai_update 时，跨语言标题只能翻译动作和语法词"
                            "（发布/推出/成立/披露/超过等），其余具体细节必须逐字照抄原文的英文单词，"
                            "不得翻译成中文名词。校验规则：标题里出现的每个英文单词都必须能在同一条原文句子里找到；"
                            "标题里除动作词和虚词外的每个中文名词，都必须对应原文里出现过的英文词，"
                            "凡是原文没有对应英文词的中文名词都会被判为编造并拒绝。"
                            "示例（原文 `Google DeepMind launches institute to widen the AGI debate`）："
                            "正确写法「Google DeepMind 成立 institute 以拓宽 AGI debate」"
                            "（保留 institute/debate/AGI，只翻译动作词）；"
                            "错误写法「Google DeepMind 成立研究院以拓宽 AGI 辩论」"
