...
- `content_type=ai_update` 使用独立发布性门禁，不要求正式发布等硬新闻动作，但仍必须有明确模型/项目主体、机械可核验的实验/榜单/数值细节以及逐条 quote、规范 URL 和保护锚点绑定；泛标题、推广、虚构结果、确定性行业结论和类型篡改必须拒绝，`fact_event` 继续使用原硬新闻门禁。
